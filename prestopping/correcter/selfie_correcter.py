...
class Correcter(object):
    ...
    def get_clean_batch(self, ids, images, labels, loss_array):

        clean_batch = MiniBatch()
        num_clean_instances = int(np.ceil(float(len(ids)) * (1.0 - self.reader.noise_rate)))
    ...
    def patch_clean_with_corrected_sample_batch(self, ids, images, labels, loss_array):
        # 1. get clean
        clean_batch = self.get_clean_batch(ids, images, labels, loss_array)
