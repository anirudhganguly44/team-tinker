...
from correcter.activebias_sampler import *

class ActiveBias(object):

    def __init__(self, reader, configuration, smoothness=0.2):
        self.reader = reader
        ...
        with graph.as_default():
            with tf.device('/gpu:' + str(self.configuration.gpu_id)):
                with tf.Session(config=config) as sess:
                    ...
                    start = time.time()
                    sampler = Sampler(self.reader.get_num_training(), self.reader.get_num_labels(), smoothness=self.smoothness)
                    ...
                    self.training(sess, trainer, sampler, training_log)
    ...
    def training(self, sess, trainer, sampler, training_log):
