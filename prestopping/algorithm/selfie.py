# we refer the source code of original author, "SELFIE: Refurbishing Unclean Sample for Robust Deep Learning"
...
from correcter.selfie_correcter import *
            ...
            with tf.device('/gpu:' + str(self.configuration.gpu_id)):
                with tf.Session(config=config) as sess:
                    ...
                    for i in range(self.restart + 1):
                        ...
                        sess.run(trainer.init_op)
                        print(colored("[LOG]", "blue"), colored(("[SELFIE training procedure] starts " + str(self.configuration.total_epoch) + " epochs"), "green"))
                        print(colored("[LOG]", "blue"), "[Parameters] history length:", self.queue_size, ", threshold:", self.threshold)
        ...
        for epoch in range(self.configuration.total_epoch):
            ...
            if epoch < self.configuration.warm_up_epoch:
                ...
                for i in range(self.reader.train_patcher.num_iters_per_epoch):
                    ...
                    # inference for loss
                    train_softmax, loss_array = sess.run([trainer.train_prob_op, trainer.train_xentropy_op], feed_dict={trainer.model.train_image_placeholder: images, trainer.model.train_label_placeholder: labels})
                    self.correcter.async_update_prediction_matrix(ids, train_softmax)

                    # low-loss sample selection
                    selected_ids, selected_images, selected_labels, num_clean, num_corrected, clean_hit, correction_hit = self.correcter.patch_clean_with_corrected_sample_batch(ids, images, labels, loss_array)
